...
SAML Integration requires the installation of the `ultraviolet-saml` package, which in turn requires `libxml2-dev` and `libxmlsec1-dev` to be installed on your system. 
...
We are also using a custom authsources.php which helps us configure the SAML IdP to reflect NYU's IdP, this is currently placed under app_data of the v10-SAML branch of ultraviolet.
...
The last step is to enable the logout redirects. In your `invenio.cfg`:
